...
const Sidebar = ({ industry, data, onUpdateData, onSubmitLead, isSubmitting }) => {
  ...
  const otherData = fields.filter(([key]) => 
    !filterKeys.includes(key.toLowerCase()) && 
    !['crm_sync', 'lead_score', 'follow_up', 'crm_lead_name', 'last_search', 'search_status'].includes(key)
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {isRealEstate && crmStatus && (
        <div className={`rounded-2xl p-4 shadow-lg border transition-all duration-500 ${
          crmStatus.includes('Synced') ? 'bg-green-600 border-green-500 text-white' : 'bg-amber-500 border-amber-400 text-white animate-pulse'
        }`}>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-bold uppercase tracking-wider">Brilworks CRM</h3>
            <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-full">Sheets Sync</span>
          </div>
          <p className="text-sm font-medium">{crmStatus}</p>
          
          {leadScore && (
            <div className="mt-3 pt-3 border-t border-white/20 flex items-center justify-between">
              <span className="text-[10px] uppercase font-bold opacity-80">Lead Score</span>
              <span className={`text-xs font-black px-2 py-1 rounded ${
                leadScore === 'HOT' ? 'bg-red-500' : leadScore === 'WARM' ? 'bg-orange-500' : 'bg-slate-400'
              }`}>{leadScore}</span>
            </div>
          )}
        </div>
      ...
      {isRealEstate && filters.length > 0 && (
        <div className="bg-blue-600 rounded-2xl p-4 text-white shadow-lg shadow-blue-200/50">
          <h3 className="text-xs font-bold uppercase tracking-wider mb-3 opacity-80">Search Filters</h3>
          <div className="grid grid-cols-1 gap-2">
            {filters.map(([key, value]) => (
              <div key={key} className="bg-white/10 rounded-lg p-2 border border-white/10 flex justify-between items-center group">
                <div className="flex-1 overflow-hidden mr-2">
                  <span className="text-[10px] font-bold uppercase opacity-60 block">{key.replace(/_/g, ' ')}</span>
                  {editingKey === key ? (
                    <input 
                      autoFocus
                    ...
                    <span className="text-xs font-semibold truncate">{String(value)}</span>
                  )}
                </div>
                {editingKey !== key && (
                  <button 
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex justify-between items-center">
          ...
          <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded uppercase">Reference</span>
        </div>
          ...
          {industry.inventory && industry.inventory.length > 0 ? (
            industry.inventory.map((item, idx) => (
              <div key={idx} className="border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                <div className="text-xs font-bold text-slate-800">{item.name || item.item || item.type || item.course || item.category}</div>
                <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1">
                  {Object.entries(item).map(([k, v]) => k !== 'name' && k !== 'item' && k !== 'type' && k !== 'course' && k !== 'category' && (
                    <span key={k} className="text-[10px] text-slate-500 bg-slate-50 px-1.5 py-0.5 rounded">
            ...
            <p className="text-xs text-slate-400 italic text-center py-4">Knowledge loaded in context.</p>
          )}
        </div>
      </div>
        ...
        <div className="p-4 space-y-4">
          <div className="space-y-3">
            {fields.length === 0 ? (
              ...
              (otherData.length === 0 && !data['last_search'] && !data['search_status']) ? (
                <p className="text-xs text-slate-400 italic">Conversational details will appear here.</p>
              ) : (
                <>
                  {data['search_status'] && (
                  ...
                  {otherData.map(([key, value]) => (
                    <div key={key} className="bg-slate-50 rounded-lg p-3 border border-slate-100 group relative">
                      <div className="flex justify-between items-start mb-1">
                        <div className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{key.replace(/_/g, ' ')}</div>
                        {editingKey !== key && (
                          <button 
                            onClick={() => startEditing(key, value)}
